...
local function ShootMurder()
    ...
    -- 1. Экипируем Gun если есть
    local gunTool = nil
    local backpack = Player:FindFirstChild("Backpack")
    
    if backpack then
        gunTool = backpack:FindFirstChild("Gun")
        if gunTool then
            gunTool.Parent = Player.Character
        end
    end
    
    -- Если не нашли в Backpack, проверяем в Character
    if not gunTool then
        gunTool = Player.Character:FindFirstChild("Gun")
    end
    ...
    -- 2. Ищем игрока с ножом в инвентаре
    local target = nil
    
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player then
            local pBackpack = p:FindFirstChild("Backpack")
            if pBackpack then
                local knife = pBackpack:FindFirstChild("Knife")
                if not knife then
                    for _, tool in ipairs(pBackpack:GetChildren()) do
                        if tool:IsA("Tool") and string.lower(tool.Name) == "knife" then
                            knife = tool
                            break
                        end
                    end
    ...
    -- 3. Стреляем если нашли цель
    if target and target.Character then
        local targetPart = target.Character:FindFirstChild("HumanoidRootPart") or 
                           target.Character:FindFirstChild("Torso") or
                           target.Character:FindFirstChild("Head")
        
        if targetPart and Player.Character then
            ...
            if myRoot then
                ...
                local success, errorMsg = pcall(function()
                    local characterGun = Player.Character:FindFirstChild("Gun")
                    if characterGun and characterGun:FindFirstChild("Shoot") then
                        characterGun.Shoot:FireServer(unpack(args))
                    end
                end)
                ...
                if not success then
                    ...
                    pcall(function()
                        game:GetService("Players").LocalPlayer.Character.Gun.Shoot:FireServer(unpack(args))
                    })
                end
            end
        end
